...
def build_relation_module(embed_dim=64, l2_reg=1e-4):
    ...
    x = layers.Dense(64, activation='leaky_relu', kernel_regularizer=reg)(inp)
    x = layers.LayerNormalization()(x)
    x = layers.Dense(32, activation='leaky_relu', kernel_regularizer=reg)(x)
    out = layers.Dense(1, activation=None)(x)
    return models.Model(inputs=inp, outputs=out, name="RelationModule_Best")
...
def train_relation_model(encoder, relation_module, episodes=3000, lr=2e-4):
...
cnn_encoder.save("best_relationnet_best_cnn_encoder.keras")
cnn_rel_module.save("best_relationnet_best_cnn_module.keras")
...
cnn_attention_encoder.save("best_relationnet_best_cnn_attention_encoder.keras")
cnn_attention_rel_module.save("best_relationnet_best_cnn_attention_module.keras")
...
cnn_mamba_encoder.save("best_relationnet_best_cnn_mamba_encoder.keras")
cnn_mamba_rel_module.save("best_relationnet_best_cnn_mamba_module.keras")
...
cnn_attention_mamba_encoder.save("best_relationnet_best_cnn_attention_mamba_encoder.keras")
cnn_attention_mamba_rel_module.save("best_relationnet_best_cnn_attention_mamba_module.keras")
...
for name, (enc_builder, rel_builder) in configurations.items():
    ...
    if name == 'CNN+Attn+Mamba':
        ...
        key = 'cnn_attention'
    try:
        enc.load_weights(f"best_relationnet_best_{key}_encoder.keras", by_name=True, skip_mismatch=True)
        rel.load_weights(f"best_relationnet_best_{key}_module.keras",  by_name=True, skip_mismatch=True)
    except Exception:
...
plt.tight_layout(); plt.savefig('fsl_relationnet_best_plot.png')
