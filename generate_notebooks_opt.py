...
# ====================== MAMBA BLOCK (shared, reused in each Mamba cell) ======================
MAMBA_BLOCK_CODE = """class StableMambaBlock(layers.Layer):
...
# ====================== ENCODER BUILDER TEMPLATES ======================
# Note: NO Dropout anywhere. Larger filters. Larger embed dim.
def cnn_encoder_code(name, activation='relu', use_ln=False):
    norm = "layers.LayerNormalization()" if use_ln else "layers.BatchNormalization()"
    return f"""def build_cnn_encoder(l2_reg=1e-4):
    ...
    return models.Model(inputs=inputs, outputs=out, name="{name}")"""

def cnn_attn_encoder_code(name, activation='relu', use_ln=False):
    norm = "layers.LayerNormalization()" if use_ln else "layers.BatchNormalization()"
    return f"""def build_cnn_attention_encoder(l2_reg=1e-4):
    inputs = layers.Input(shape=(NUM_BANDS, NUM_STATS))
    reg = tf.keras.regularizers.l2(l2_reg)
    x = layers.Conv1D(128, kernel_size=7, activation='{activation}', padding='same', kernel_regularizer=reg)(inputs)
    x = layers.MaxPooling1D(2)(x)
...
def cnn_mamba_encoder_code(name, activation='leaky_relu', use_ln=True):
    return f"""def build_cnn_mamba_encoder(l2_reg=1e-4):
    inputs = layers.Input(shape=(NUM_BANDS, NUM_STATS))
    reg = tf.keras.regularizers.l2(l2_reg)
    x = layers.Conv1D(128, kernel_size=7, activation='{activation}', padding='same', kernel_regularizer=reg)(inputs)
    x = layers.MaxPooling1D(2)(x)
...
def cnn_attn_mamba_encoder_code(name, activation='leaky_relu', use_ln=True):
    return f"""def build_cnn_attention_mamba_encoder(l2_reg=1e-4):
    inputs = layers.Input(shape=(NUM_BANDS, NUM_STATS))
    reg = tf.keras.regularizers.l2(l2_reg)
    x = layers.Conv1D(128, kernel_size=7, activation='{activation}', padding='same', kernel_regularizer=reg)(inputs)
    x = layers.MaxPooling1D(2)(x)
...
# ====================== PLOTTING CELL (episodic) ======================
PLOT_CELL_EPISODIC = """# --- Plotting Cell ---
histories = {
...
if 'eval_results' in locals() and eval_results:
    ...
    plt.ylabel('Accuracy (%)'); plt.grid(True); plt.show()"""

# ==============================================================
# ==================== PROTONET BACKUP =========================
# ==============================================================

PROTO_SETUP = SETUP_BASE + """
...
class PrototypicalNetwork(tf.keras.Model):
    ...
    def call(self, support_x, support_y, query_x, n_way, k_shot):
        support_emb = self.encoder(support_x, training=True)
        query_emb   = self.encoder(query_x,   training=True)
        d = tf.shape(support_emb)[-1]
        prototypes = tf.reduce_mean(tf.reshape(support_emb, (n_way, k_shot, d)), axis=1)
        dists = tf.reduce_sum(
            tf.square(tf.expand_dims(query_emb, 1) - tf.expand_dims(prototypes, 0)), axis=-1
        )
        return -dists   # logits: lower distance = higher score

def train_step_proto(model, optimizer, sx, sy, qx, qy, n_way, k_shot):
    with tf.GradientTape() as tape:
        logits = model(sx, sy, qx, n_way, k_shot)
        loss   = tf.reduce_mean(
...
def train_protonet(encoder, episodes=3000, lr=3e-4):
    proto = PrototypicalNetwork(encoder)
    # Cosine-decay learning rate for smooth convergence
    ...
    for ep in range(1, episodes + 1):
        ...
        loss, acc = train_step_proto(proto, optimizer, sx, sy, qx, qy, N_WAY, K_SHOT)
        loss_hist.append(float(loss))
        acc_hist.append(float(acc))
        if ep % 300 == 0:
            recent_acc = np.mean(acc_hist[-100:]) * 100
            print(f"  Ep {ep:4d}/{episodes} | Loss: {loss:.4f} | Recent Acc: {recent_acc:.1f}%")
    return loss_hist, acc_hist"""

PROTO_CNN_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_encoder_code("CNN_Encoder_OPT") + """

cnn_encoder = build_cnn_encoder()
print(f"CNN encoder params: {cnn_encoder.count_params():,}")
print("Training CNN ProtoNet (optimized)...")
loss_cnn, acc_cnn = train_protonet(cnn_encoder, episodes=3000)
cnn_encoder.save("best_opt_protonet_cnn.keras")
print(f"CNN Final avg acc: {np.mean(acc_cnn[-200:])*100:.2f}%")"""

PROTO_ATTN_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_attn_encoder_code("CNN_Attn_Encoder_OPT") + """

cnn_attention_encoder = build_cnn_attention_encoder()
print(f"CNN+Attn encoder params: {cnn_attention_encoder.count_params():,}")
print("Training CNN+Attention ProtoNet (optimized)...")
loss_attention, acc_attention = train_protonet(cnn_attention_encoder, episodes=3000)
cnn_attention_encoder.save("best_opt_protonet_cnn_attention.keras")
print(f"CNN+Attn Final avg acc: {np.mean(acc_attention[-200:])*100:.2f}%")"""

PROTO_MAMBA_CELL = MAMBA_BLOCK_CODE + "\n\n" + "tf.keras.backend.clear_session()\n\n" + cnn_mamba_encoder_code("CNN_Mamba_Encoder_OPT") + """

cnn_mamba_encoder = build_cnn_mamba_encoder()
print(f"CNN+Mamba encoder params: {cnn_mamba_encoder.count_params():,}")
print("Training CNN+Mamba ProtoNet (optimized)...")
loss_mamba, acc_mamba = train_protonet(cnn_mamba_encoder, episodes=3000)
cnn_mamba_encoder.save("best_opt_protonet_cnn_mamba.keras")
print(f"CNN+Mamba Final avg acc: {np.mean(acc_mamba[-200:])*100:.2f}%")"""

PROTO_ATTN_MAMBA_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_attn_mamba_encoder_code("CNN_AttnMamba_Encoder_OPT") + """
...
print("Training CNN+Attn+Mamba ProtoNet (optimized)...")
loss_attn_mamba, acc_attn_mamba = train_protonet(cnn_attention_mamba_encoder, episodes=3000)
cnn_attention_mamba_encoder.save("best_opt_protonet_cnn_attention_mamba.keras")
print(f"CNN+Attn+Mamba Final avg acc: {np.mean(acc_attn_mamba[-200:])*100:.2f}%")"""

PROTO_TEST_CELL = """# --- Testing Cell: 5-Shot Holdout Evaluation over 100 trials ---
encoders_map = {
    'CNN':                   (lambda: build_cnn_encoder(),                      'best_opt_protonet_cnn.keras'),
    'CNN+Attention':         (lambda: build_cnn_attention_encoder(),             'best_opt_protonet_cnn_attention.keras'),
    'CNN+Mamba':             (lambda: build_cnn_mamba_encoder(),                 'best_opt_protonet_cnn_mamba.keras'),
    'CNN+Attn+Mamba':        (lambda: build_cnn_attention_mamba_encoder(),       'best_opt_protonet_cnn_attention_mamba.keras'),
}
eval_results = {}

for name, (builder, fname) in encoders_map.items():
    tf.keras.backend.clear_session()
    enc = builder()
    print(f"\\nEvaluating {name}...")
    try:
        enc.load_weights(fname, by_name=True, skip_mismatch=True)
    except Exception:
    ...
    for i in range(100):
        ...
        dists   = tf.reduce_sum(tf.square(tf.expand_dims(qry_emb, 1) - tf.expand_dims(protos, 0)), axis=-1)
...
protonet_opt_cells = [
    ("markdown", "# FSL ProtoNet — Optimized Backup\\n\\nKey changes vs original:\\n- **No Dropout** (removed throughout)\\n- Larger CNN: 128→256→256 filters, kernel 7/5/3\\n- Embedding dim: 64 (was 32)\\n- Episodes: 3000 (was 1000) with cosine LR decay\\n- Random class selection per episode (was fixed)\\n- Mamba d_state: 32 (was 16), 4 attention heads\\n- 10 query samples per episode"),
    ("code", PROTO_SETUP),
    ("code", PROTO_CNN_CELL),
    ("code", PROTO_ATTN_CELL),
    ("code", PROTO_MAMBA_CELL),
    ("code", PROTO_ATTN_MAMBA_CELL),
    ("code", PROTO_TEST_CELL),
    ("code", PLOT_CELL_EPISODIC),
]
...
REL_CNN_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_encoder_code("CNN_Encoder_REL_OPT", activation='leaky_relu') + """

cnn_encoder    = build_cnn_encoder()
cnn_rel_module = build_relation_module(embed_dim=EMBED_DIM)
print(f"CNN encoder params: {cnn_encoder.count_params():,}")
print("Training CNN RelationNet (optimized)...")
loss_cnn, acc_cnn = train_relation_model(cnn_encoder, cnn_rel_module, episodes=3000)
cnn_encoder.save("best_opt_relationnet_cnn_encoder.keras")
cnn_rel_module.save("best_opt_relationnet_cnn_module.keras")
print(f"CNN Final avg acc: {np.mean(acc_cnn[-200:])*100:.2f}%")"""

REL_ATTN_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_attn_encoder_code("CNN_Attn_Encoder_REL_OPT", activation='leaky_relu') + """

cnn_attention_encoder    = build_cnn_attention_encoder()
cnn_attention_rel_module = build_relation_module(embed_dim=EMBED_DIM)
print(f"CNN+Attn encoder params: {cnn_attention_encoder.count_params():,}")
print("Training CNN+Attention RelationNet (optimized)...")
loss_attention, acc_attention = train_relation_model(cnn_attention_encoder, cnn_attention_rel_module, episodes=3000)
...
REL_MAMBA_CELL = MAMBA_BLOCK_CODE + "\n\n" + "tf.keras.backend.clear_session()\n\n" + cnn_mamba_encoder_code("CNN_Mamba_Encoder_REL_OPT") + """

cnn_mamba_encoder    = build_cnn_mamba_encoder()
cnn_mamba_rel_module = build_relation_module(embed_dim=EMBED_DIM)
print(f"CNN+Mamba encoder params: {cnn_mamba_encoder.count_params():,}")
print("Training CNN+Mamba RelationNet (optimized)...")
loss_mamba, acc_mamba = train_relation_model(cnn_mamba_encoder, cnn_mamba_rel_module, episodes=3000)
...
REL_ATTN_MAMBA_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_attn_mamba_encoder_code("CNN_AttnMamba_Encoder_REL_OPT") + """
...
loss_attn_mamba, acc_attn_mamba = train_relation_model(cnn_attention_mamba_encoder, cnn_attention_mamba_rel_module, episodes=3000)
...
REL_TEST_CELL = """# --- Testing Cell: 5-Shot Holdout Evaluation over 100 trials ---
configurations = {
...
relationnet_opt_cells = [
    ("markdown", "# FSL RelationNet — Optimized Backup\\n\\nKey changes vs original:\\n- **No Dropout** (removed throughout)\\n- Larger CNN: 128→256→256 filters, kernel 7/5/3\\n- Embedding dim: 64 (was 32)\\n- Deeper Relation Module: 512→256→128→64→1 (was 64→32→1)\\n- Episodes: 3000 with cosine LR decay\\n- Random class selection per episode\\n- Mamba d_state: 32, 4 attention heads\\n- 10 query samples per episode"),
    ("code", REL_SETUP),
    ("code", REL_CNN_CELL),
    ("code", REL_ATTN_CELL),
    ("code", REL_MAMBA_CELL),
    ("code", REL_ATTN_MAMBA_CELL),
    ("code", REL_TEST_CELL),
    ("code", PLOT_CELL_EPISODIC),
]
...
MAML_SETUP = SETUP_BASE + """

INNER_STEPS = 5      # Inner loop gradient steps (was 1 implicit)
INNER_LR    = 0.05   # Inner loop learning rate alpha (was 0.01)
...
MAML_CNN_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_encoder_code("CNN_Encoder_MAML_OPT", activation='leaky_relu', use_ln=True) + """
...
MAML_ATTN_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_attn_encoder_code("CNN_Attn_Encoder_MAML_OPT", activation='leaky_relu', use_ln=True) + """
...
MAML_MAMBA_CELL = MAMBA_BLOCK_CODE + "\n\n" + "tf.keras.backend.clear_session()\n\n" + cnn_mamba_encoder_code("CNN_Mamba_Encoder_MAML_OPT") + """
...
MAML_ATTN_MAMBA_CELL = "tf.keras.backend.clear_session()\n\n" + cnn_attn_mamba_encoder_code("CNN_AttnMamba_Encoder_MAML_OPT") + """
...
MAML_TEST_CELL = """# --- Testing Cell: 5-Shot Holdout Evaluation with Test-Time Adaptation ---
maml_models = {
...
for name, builder in maml_models.items():
    ...
    fname = f"best_opt_maml_{name.lower().replace('+','_').replace(' ','_')}_weights.weights.h5"
    # Special handling for cnn+attn+mamba because it was saved as cnn_attention_mamba
    if name == 'CNN+Attn+Mamba':
        fname = "best_opt_maml_cnn_attention_mamba_weights.weights.h5"
...
maml_opt_cells = [
    ("markdown", "# FSL MAML — Optimized Backup\\n\\nKey changes vs original:\\n- **No Dropout** (removed throughout)\\n- Larger CNN: 128→256→256 filters, kernel 7/5/3\\n- Embedding dim: 64 (was 32)\\n- **5 inner loop steps** (was 1), alpha=0.05 (was 0.01)\\n- Episodes: 3000 with cosine LR decay, outer LR=1e-3\\n- Random class selection per episode\\n- Mamba d_state: 32, 4 attention heads\\n- Dummy forward pass to build model before training"),
    ("code", MAML_SETUP),
    ("code", MAML_CNN_CELL),
    ("code", MAML_ATTN_CELL),
    ("code", MAML_MAMBA_CELL),
    ("code", MAML_ATTN_MAMBA_CELL),
    ("code", MAML_TEST_CELL),
    ("code", PLOT_CELL_EPISODIC),
]

# ====================== GENERATE ALL ======================
create_notebook("fsl_protonet_opt.ipynb",    protonet_opt_cells)
create_notebook("fsl_relationnet_opt.ipynb", relationnet_opt_cells)
create_notebook("fsl_maml_opt.ipynb",        maml_opt_cells)
print("\\nAll optimized backup notebooks created!")
print("Original notebooks (fsl_protonet.ipynb, fsl_relationnet.ipynb, fsl_maml.ipynb) are UNTOUCHED.")
